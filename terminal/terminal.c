...
#define BUFSIZE 200 // user can enter up to this number of characters
...
if ( (Buf = Alloc(BUFSIZE)) == NULL ) return;
...
while (1)
    ...
    if (FD_ISSET(Master, &fds))
        {
        BeRead = read(Master, Buf, BUFSIZE);
        if (BeRead <= 0) break;
    ...
    if (FD_ISSET(STDIN_FILENO, &fds))
        {
        BeRead = read(STDIN_FILENO, Buf, BUFSIZE-1);
        if (BeRead <= 0) break;
        Buf[BeRead] = '\0';
